    ...
    <div className="py-2 flex items-center gap-2 justify-between">
      <div className="min-w-0 flex-1">
        <div className="flex min-w-0 flex-wrap items-center gap-1">
          <span className="truncate text-sm min-w-0 flex-1" title={s.path}>
            {s.path}
          </span>
          {mark && (
            <span title="Content match" className="text-emerald-600 text-xs shrink-0">
              ●
            </span>
          )}
          {tagsLoading ? (
            ...
            <span className="text-xs text-red-500">Tags unavailable</span>
          ) : (
            hasTags &&
            tags?.map((tag) => (
              <Badge
